...
function MonthlyMetrics({ station, record, money }) {
  const rows = record.monthly_metrics || [];
  const latest = rows[0] || record.latest_monthly_metric;
  const count = (value) => value == null ? "—" : Number(value).toLocaleString("en-IN");
  return <Panel title="Monthly station data" subtitle="Read-only station history from the complete monthly master; newest month is always first."><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><StationMetric label="Footfall" value={count(latest?.passenger_footfall)} subtext={latest?.metric_month || "No monthly entry yet"} /><StationMetric label="UTS tickets" value={count(latest?.uts_tickets)} subtext="Unreserved tickets" /><StationMetric label="UTS earnings" value={latest?.uts_earnings == null ? "—" : money(latest.uts_earnings)} subtext="Unreserved earnings" /><StationMetric label="PRS tickets" value={count(latest?.prs_tickets)} subtext="Reserved tickets" /><StationMetric label="PRS earnings" value={latest?.prs_earnings == null ? "—" : money(latest.prs_earnings)} subtext="Reserved earnings" /></div><div className="soft-scroll mt-4 overflow-x-auto rounded-lg border border-line"><table className="min-w-[760px] w-full text-sm"><thead className="bg-surfaceStrong text-left text-xs font-black text-muted"><tr><th className="px-3 py-2">Month</th><th className="px-3 py-2 text-right">Footfall</th><th className="px-3 py-2 text-right">UTS tickets</th><th className="px-3 py-2 text-right">UTS earnings</th><th className="px-3 py-2 text-right">PRS tickets</th><th className="px-3 py-2 text-right">PRS earnings</th></tr></thead><tbody>{rows.slice(0, 12).map((row) => <tr key={row.metric_id} className="border-t border-line"><td className="px-3 py-2 font-bold text-ink">{row.metric_month}</td><td className="px-3 py-2 text-right">{count(row.passenger_footfall)}</td><td className="px-3 py-2 text-right">{count(row.uts_tickets)}</td><td className="px-3 py-2 text-right">{row.uts_earnings == null ? "—" : money(row.uts_earnings)}</td><td className="px-3 py-2 text-right">{count(row.prs_tickets)}</td><td className="px-3 py-2 text-right">{row.prs_earnings == null ? "—" : money(row.prs_earnings)}</td></tr>)}</tbody></table></div></Panel>;
}
        ...
        <div className="space-y-4">
          ...
          <KeyValueGrid
            rows={[
              ...
              ["Baseline Footfall", dailyFootfall(station)],
              ["Total Platforms", totalPlatforms],
              ["Baseline Earnings / Day", station.earnings_per_day],
