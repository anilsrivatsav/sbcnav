...
export default function ContractsPage() {
  ...
  const visibleRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      return statusOk && typeOk && (!needle || JSON.stringify(row).toLowerCase().includes(needle));
    });
  }, [rows, search, category, status, cateringStatus, cateringType]);
  ...
  return <main className="min-h-screen bg-[#f7f9fc] px-5 py-8 text-slate-900 md:px-10"><div className="mx-auto max-w-7xl">
    ...
    <div className="mb-5 flex flex-wrap items-center gap-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">{category === "publicity" && statuses.map(([key, label]) => <button key={key} onClick={() => setStatus(key)} className={`rounded-xl px-4 py-2 text-sm font-semibold ${status === key ? "bg-indigo-600 text-white" : "text-slate-600 hover:bg-slate-100"}`}>{label}</button>)}{category === "catering" && [["running", "Running / Active", cateringRunningRows.length], ["unawarded", "Unawarded / Other", datasets.catering.filter((row) => row.status === "unawarded").length]].map(([key, label, count]) => <button key={key} onClick={() => { setCateringStatus(key); setCateringType("all"); }} className={`rounded-xl px-4 py-2 text-sm font-semibold ${cateringStatus === key ? "bg-indigo-600 text-white" : "text-slate-600 hover:bg-slate-100"}`}>{label} {count}</button>)}<label className="relative ml-auto min-w-[240px] flex-1 sm:max-w-sm"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search contracts, station, train, policy..." className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-sm outline-none focus:border-indigo-400" /></label></div>
    {category === "catering" && cateringStatus === "running" && <div className="mb-5 flex flex-wrap items-center gap-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm"><span className="mr-1 text-xs font-bold uppercase tracking-wide text-slate-500">Type</span><button onClick={() => setCateringType("all")} className={`rounded-xl px-3 py-2 text-xs font-bold ${cateringType === "all" ? "bg-indigo-600 text-white" : "text-slate-600 hover:bg-slate-100"}`}>All types {cateringRunningRows.length}</button>{cateringTypes.map((type) => <button key={type} onClick={() => setCateringType(type)} className={`rounded-xl px-3 py-2 text-xs font-bold ${cateringType === type ? "bg-indigo-600 text-white" : "text-slate-600 hover:bg-slate-100"}`}>{type} {cateringRunningRows.filter((row) => String(row.type_of_unit || "").toLowerCase() === type.toLowerCase()).length}</button>)}</div>}
