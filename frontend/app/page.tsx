          ...
          {view === "contracts" ? (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-1.5"><span className="mr-1 text-[11px] font-black uppercase tracking-[0.14em] text-muted">Type</span>{cateringTypeChips.map((chip) => <button key={chip.value} type="button" onClick={() => setCateringType(chip.value)} className={cx("rounded-full border px-3 py-1.5 text-xs font-black transition hover:border-accent", cateringType === chip.value ? "border-accent bg-accent text-white" : "soft-control text-ink")}>{chip.label} <span className="ml-1 opacity-75">{chip.count}</span></button>)}</div>
