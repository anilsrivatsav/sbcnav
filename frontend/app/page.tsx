        ...
        <section className="soft-scroll min-w-0 space-y-4 lg:h-full lg:overflow-auto lg:pr-2">
          ...
          {view === "contracts" ? (
            <div className="space-y-4">
              ...
              <>
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-1.5"><span className="mr-1 text-[11px] font-black uppercase tracking-[0.14em] text-muted">Status</span><button type="button" onClick={() => setContractTab("active")} className={cx("rounded-full border px-3 py-1.5 text-xs font-black transition hover:border-accent", contractTab === "active" ? "border-accent bg-accent text-white" : "soft-control text-ink")}>Running / Active <b className="ml-1 opacity-75">{filteredContracts.active.length}</b></button><button type="button" onClick={() => setContractTab("other")} className={cx("rounded-full border px-3 py-1.5 text-xs font-black transition hover:border-accent", contractTab === "other" ? "border-accent bg-accent text-white" : "soft-control text-ink")}>Unawarded / Other <b className="ml-1 opacity-75">{filteredContracts.other.length}</b></button><button type="button" onClick={() => setContractTab("earnings")} className={cx("rounded-full border px-3 py-1.5 text-xs font-black transition hover:border-accent", contractTab === "earnings" ? "border-accent bg-accent text-white" : "soft-control text-ink")}>Payments <b className="ml-1 opacity-75">{filteredContracts.earnings.length}</b></button></div>
                <div className="flex flex-wrap items-center gap-1.5"><span className="mr-1 text-[11px] font-black uppercase tracking-[0.14em] text-muted">Type</span>{cateringTypeChips.map((chip) => <button key={chip.value} type="button" onClick={() => { setCateringType(chip.value); setCateringStation("all"); }} className={cx("rounded-full border px-3 py-1.5 text-xs font-black transition hover:border-accent", cateringType === chip.value ? "border-accent bg-accent text-white" : "soft-control text-ink")}>{chip.label} <span className="ml-1 opacity-75">{chip.count}</span></button>)}</div>
