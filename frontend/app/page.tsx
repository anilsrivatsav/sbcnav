...
export default function Page() {
  ...
  const publicityPolicyOptions = Array.from(new Set(publicityContracts.map((row) => String(row.policy_code || "").trim()).filter(Boolean))).sort();
  ...
  return (
    <main className="min-h-screen bg-[var(--page-background)] text-ink">
          ...
          {view === "contracts" ? (
            <div className="space-y-4">
              <Tabs
                ...
                onChange={(value) => {
                  ...
                  if (value === "catering") {
                    ...
                    // A contract search is shared by Catering and Publicity.
                    // Clear it when switching families so a prior catering
                    // query cannot silently reduce the Publicity count.
                    setSearch((prev) => ({ ...prev, contracts: "" }));
                    setPublicityStatus("all");
                    setPublicityPolicy("all");
                    setPublicityStation("all");
                  }
              ...
              {contractFamily === "publicity" ? (
                <>
                  <div className="soft-surface grid gap-3 rounded-xl border border-line p-3 md:grid-cols-4 md:items-end">
                    <label className="grid gap-1 text-xs font-black text-muted"><span className="text-[11px] uppercase tracking-[0.14em]">Status</span><select value={publicityStatus} onChange={(event) => { setPublicityStatus(event.target.value); setPublicityPolicy("all"); setPublicityStation("all"); }} className="soft-inset h-10 rounded-lg border border-line px-3 text-sm font-bold text-ink outline-none focus:border-accent">{["all", "running", "completed", "cancelled"].map((status) => <option key={status} value={status}>{status === "all" ? `All contracts (${publicityContracts.length})` : `${pretty(status)} (${publicityContracts.filter((row) => normalizeText(row.status) === status).length})`}</option>)}</select></label>
                  ...
                  <Panel title="Publicity contracts" subtitle="Select a row to open the contract workspace and payment schedule." action={<Button size="sm" variant="secondary" onClick={() => setView("settings")}><SettingsIcon size={14} /> Manage in Settings</Button>}>
                    <DataTable
                      key={`publicity-${publicityStatus}-${publicityPolicy}-${publicityStation}`}
              ...
              >
                ...
                <div className="mt-4">
                  <DataTable
                    key={`catering-${contractTab}-${cateringType}-${cateringStation}`}
      ...
      <Modal
        ...
        onClose={closeModal}
        side={modal.type === "publicity" || modal.type === "unit" || modal.type === "commercial"}
      >
        {modal.type === "station" ? (
          <Station360
            record={modal.record}
